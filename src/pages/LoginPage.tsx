...
function mapAuthError(error: unknown): string {
  if (error && typeof error === 'object' && 'message' in error) {
    ...
    if (msg.includes('user not found') || msg.includes('not registered') || msg.includes('no user found')) {
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const onboarded = await login(email.trim(), password);
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-surface px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="card p-8">
          ...
          <p className="mt-1.5 text-ink-500">Sign in to continue your career journey.</p>
